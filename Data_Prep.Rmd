...
results_manual = tibble::tibble(
  `Train Error` = sqrt(rf_manual$mse),
  `Test error` = sqrt(rf_manual$test$mse),
  ntrees = 1:rf_manual$ntree
) %>% gather(Metric, RMSE, -ntrees)

results_website = tibble::tibble(
  `Train Error` = sqrt(rf_website$mse),
  `Test error` = sqrt(rf_website$test$mse),
  ntrees = 1:rf_website$ntree
) %>% gather(Metric, RMSE, -ntrees)

# Plot Manual Leads Train and Test RMSE over number of trees
results_manual %>% ggplot( aes(ntrees, RMSE, color = Metric)) + 
    geom_line(aes()) +
...
results_website %>% ggplot( aes(ntrees, RMSE, color = Metric)) + 
    geom_line(aes()) +
    labs(x = "Number of Trees", y = "RMSE", color = "RMSE Type") +
    ggtitle("Train/Test RMSE  over Number of Trees for Website Leads") +
    theme(
    plot.title = element_text(size = 14),
    axis.title = element_text(size = 12),
    axis.text = element_text(size = 10),
