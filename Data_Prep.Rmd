...
for (ntrees in seq(1,100,3)){
  print(ntrees)
  rf.manual <- randomForest(NextDayLeads.Manual ~ . , 
                            data = manual_train, 
                            ntree = ntrees, 
                            mtry = round( sqrt( ncol(manual_train) - 1)),
                            importance = TRUE, 
                            type = Regression, 
                            nodesize = 5,
                            keep_tree = FALSE)
  rf.website <- randomForest(NextDayLeads.Website ~ ., 
                             data = website_train, 
                             ntree = ntrees, 
                             mtry = round( sqrt( ncol(website_train) - 1)),
                             importance = TRUE, 
                             type = Regression, 
                             nodesize = 5,
                             keep_tree = FALSE)
